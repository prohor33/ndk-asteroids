...
void PhysicsEngine::computeCollisions() {
  ...
  for (cii=objContainer.begin(); cii!=objContainer.end();) {
    ...
    if ((*cii)->p().x() - (*cii)->size().x()*3/2 > GLogic->getHScrSize().x() ||
        (*cii)->p().x() + (*cii)->size().x()*3/2 < -GLogic->getHScrSize().x() ||
        (*cii)->p().y() - (*cii)->size().y()*3/2 > GLogic->getHScrSize().y() ||
        (*cii)->p().y() + (*cii)->size().y()*3/2 < -GLogic->getHScrSize().y()) {
      objContainer.erase(cii);
      continue;
    }
    ...
    for (cii2=objContainer.begin(); cii2!=objContainer.end(); ++cii2) {
      if ((*cii) == (*cii2))
        continue;
      if ((*cii)->obj_type() == (*cii2)->obj_type())
        continue;
      switch ((*cii)->obj_type()) {
      case SpaceObject::BULLET:
        switch ((*cii2)->obj_type()) {
        // with
...
//
//  if (o1->obj_type() == SpaceObject::SPACE_SHIP && output.size() > 0 ) {
...
void PhysicsEngine::deleteAllObjects() {
  ...
  for (cii=objContainer.begin(); cii!=objContainer.end(); ++cii) {
    if ((*cii)->obj_type() == SpaceObject::SPACE_SHIP) {
      copy = (*cii);
      break;
    }
  }
